...
class VariableRecoveryFast(ForwardAnalysis, VariableRecoveryBase):  #pylint:disable=abstract-method
    ...
    def _pre_analysis(self):
        ...
        # Get calling conventions for library functions
        CallingConventionAnalysis.recover_calling_conventions(self.project, kb=self.kb, fast=True, libfuncs_only=True)
    ...
    def _process_block(self, state, block):  # pylint:disable=no-self-use
        ...
        if block.addr in self._node_to_cc:
            ...
            if cc is not None:
                if cc.sp_delta is None:
                    l.warning("Unknown SP delta at the end of block %#x. Calling convention analysis probably failed.",
                              block.addr)
                    return
